...
export async function sendParentalConsentEmail(childEmail, childUsername, parentEmail, consentToken) {
  ...
  const BASE_URL = process.env.FRONTEND_URL || 'https://get2know.me';
  const consentHtml = `
   <div style="width:100%;background:#f4f4f4;padding:32px 0;text-align:center;">
    <div style="max-width:480px;margin:0 auto;background:#fff;border-radius:10px;padding:32px 24px 24px 24px;font-family:sans-serif;box-shadow:0 2px 8px rgba(0,0,0,0.07);text-align:center;">
      <div style="margin-bottom:24px;text-align:center;">
        <img src="${BASE_URL}/get2knowme_logo_png.png" alt="Get2KnowMe Logo" style="height:90px;margin-bottom:8px;" />
      ...
      <div style="text-align:center;">
        ...
        <a href="${BASE_URL}" target="_blank" style="display:inline-block;padding:10px 20px;background:#007bff;color:#fff;text-decoration:none;border-radius:5px;margin-top:12px;">Visit Our Site Here</a>
      </div>
      <p style="text-align:center;">By giving your consent, you attest that you are the parent or guardian of the user indicated above. You attest that you have read the <a href="${BASE_URL}/policy/UserInfo" target="_blank">Privacy Policy</a> and <a href="${BASE_URL}/policy/terms-of-service" target="_blank">Terms of Service</a> and consent to allowing Get2KnowMe to collect and use this user's data as is outlined in those documents.</p>
      <p style="margin-top:24px;text-align:center;">
        <a href="${BASE_URL}/api/users/consent?token=${encodeURIComponent(consentToken)}" target="_blank" style="display:inline-block;padding:12px 28px;background:#28a745;color:#fff;text-decoration:none;font-weight:bold;border-radius:5px;margin-right:12px;">I Consent</a>
        <a href="${BASE_URL}/api/users/consent/declined?token=${encodeURIComponent(consentToken)}" target="_blank" style="display:inline-block;padding:12px 28px;background:#dc3545;color:#fff;text-decoration:none;font-weight:bold;border-radius:5px;">I Do Not Consent</a>
      </p>
    </div>
  </div>
  `;
